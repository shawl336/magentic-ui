...
class FileSurfer(BaseChatAgent, Component[FileSurferConfig]):
    ...
    你是一个为用户提供帮助的AI助手，你可以阅读文件并回答用户的问题。

    你要选择并使用合适且可用的方法函数来帮助用户完成他的请求:
    - open_path: 打开一个文件或目录
    - list_current_directory: 列出当前目录下的文件
    - page_up: 在已经打开的文件中向上滚动翻页
    - page_down: 在已经打开的文件中向下滚动翻页
    - find_on_page_ctrl_f: 在已经打开的文件中查找文本
    - find_file: 返回一个匹配查询的文件列表，如果找到完全匹配的文件，则会打开它
    如果用户的查询与文件无关，或者你已经打开了文件，你不需要再使用工具。

    帮助提示:
    - 不要重复同样的动作两次。
    - 如果你已经打开了一个文件，你不需要再次打开它，而是返回一个字符串回答告诉用户你已经打开了文件，并回答他们的请求。
    - 要分析一个文件，你需要先打开它，然后直接返回一个不需要调用工具的string形式的回答。

    当给定一个用户的查询时，使用可用的且合适的函数来帮助用户完成他们的请求。
    今天的日期是: {date_today}
    """
    ...
    async def on_messages_stream(
        ...
        try:
            ...
            context_message = UserMessage(
                source="user",
                content=f"你的文件查看器当前打开的是文件或文件夹是'{self._browser.page_title}'，路径是'{self._browser.path}'。",
            )
            ...
            if isinstance(response, str):
                ...
                for function_call in function_calls:
                    ...
                    try:
                        ...
                        error_str = f"文件浏览器在解码JSON参数'{e}'的时候遇到了错误。"
                        yield Response(
                            chat_message=TextMessage(
                    ...
                    self._chat_history.append(
                        AssistantMessage(
                            content=f"在'{self._browser.path}'该路径下, 我们提议执行如下动作：{tool_call_msg}",
                            source=self.name,
                        )
                    )
