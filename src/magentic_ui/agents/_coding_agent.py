        ...
        try:
            for cb in code_block_list:
                ...
                if encountered_exception or result is None:
                    code_output = f"当执行以下代码时出现异常: {code_output}"
                elif code_output.strip() == "":
                    # No output
                    code_output = f"当前脚本已被执行但是没有输出任何内容到控制台。 它的POSIX退出代码是： {result.exit_code}。 如果你期望脚本有输出，请修正脚本以保证stdout有内容打印。"
                elif exit_code != 0:
                    # Error
                    code_output = f"当前脚本已被执行, 但是发生了错误 (POSIX退出代码是: {result.exit_code})\n它的输出是:\n{result.output}"
...
class CodingAgentConfig(BaseModel):
    ...
    coding_tool: NamedMcpServerParams
...
class CodingAgent(BaseChatAgent, Component[CodingAgentConfig]):
    ...
    DEFAULT_DESCRIPTION = """
    一个可以写代码的智能体。它可以使用它的语言能力来分析你的代码需求、生成代码、保存代码到文件。
    ...
    你装备有一个强大的代码工具，它懂得C/C++、Java、Python、Shell、SQL、HTML、CSS、JavaScript、TypeScript、React、Vue、Angular、Node.js、Express、Django、Flask、FastAPI、MySQL、PostgreSQL、SQLite等几乎所有常见的编程语言，你可以使用该工具帮助你完成任何代码相关的任务。
    ...
    * 例子只是为了给抽象的语言描述做一个大致的具体说明，实际上你需要根据上下文和自身的知识做出最合适的回答。
    ...
    <代码生成>
    关于代码生成，你需要谨记并遵循如下规定：
    - 对于简单的代码需求，你可以直接用自己的知识为用户生成满足需求的代码，而不需要使用额外的代码工具。
    - 除简单的代码需求外，你总是使用代码工具来完成用户的其他代码需求。
    - 使用代码工具时，你不需要为用户生成任何代码，所有的代码都将由代码工具生成。
    - 代码工具生成的代码，可能直接保存在文件中，此时你只需要告诉用户代码已经生成，并告诉用户文件路径。
    ...
    def __init__(
        ...
        if work_dir is None:
            ...
            self._work_dir = Path(work_dir) / "coding"
        ...
        if code_executor:
            ...
            self._code_executor = DockerCommandLineCodeExecutor(
                ...
                init_command=". /data/gemini_cli/run.sh",
            )
        ...
        try:
            ...
            if self._summarize_output and executed_code:
                ...
                final_response_msg = TextMessage(
                    source=self.name,
                    metadata={"internal": "yes"},
                    content=combined_output or "无输出。",
                )
            ...
            self._chat_history.append(
                TextMessage(
                    content=f"执行以下代码时发生错误： {e}",
                    source=self.name,
                )
            )
            yield Response(
                chat_message=TextMessage(
                    content=f"coder智能体发生如下错误： {e}",
                    source=self.name,
                    metadata={"internal": "no"},
                ),
                inner_messages=inner_messages,
    ...
    def _to_config(self) -> CodingAgentConfig:
        ...
        return CodingAgentConfig(
            ...
            coding_tool=self.coding_workbench.server_params[0],
    ...
    def _from_config(cls, config: CodingAgentConfig) -> Self:
        ...
        return cls(
            ...
            coding_tool=config.coding_tool,
