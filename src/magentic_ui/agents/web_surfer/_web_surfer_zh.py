...
class WebSurfer(BaseChatAgent, Component[WebSurferConfig]):
    ...
    DEFAULT_DESCRIPTION = """
    websurfer智能体可以访问并控制网页浏览器。
    它可以分析理解图片并使用图片帮助它完成任务。
    
    在每一步中，你交给该智能体一个任务，它会有序地执行多个动作直到它认为任务完成或者达到某个阈值条件而停止。
    这些动作有:
    - 访问一个网页的url
    - 使用配置的搜索引擎进行网页搜索
    - 和网页进行交互: 点击一个按钮，悬浮鼠标到某个按钮上，在字段(field)中输入文字，滚动网页，在下拉列表(dropdown)中种选择一个选项
    - 从网页下载文件，从本地文件系统上传一个文件
    - 点击键盘按键和网页进行交互
    - 根据整个网页内容回答问题(不只是网页视点(viewport)中的部分，是整个网页的内容)
    - 等待网页载入
    - 操作网页的标签页(tab): 关闭标签页，切换标签页，新建标签页
    - 刷新网页

    websurfer智能体会在交给它的一步任务中实际执行多个动作，但是它会在合适的时候停止动作或者超过最大规定的动作次数后停止。
    比如，如果让websurfer智能体去填写一个表格，它会输入姓名，再输入邮件，然后提交表格，最后停止。
    """
    ...
    def _get_search_url(self, query: str) -> tuple[str, str]:
        ...
        elif self.search_engine.lower() == "baidu" or self.search_engine.lower() == "百度" \
                or self.search_engine.lower() == "百度搜索" :
            domain = "baidu.com"
    ...
    async def on_messages_stream(
        ...
        assert self._page is not None, "网页应该被初始化"
        ...
        if (
            ...
            yield TextMessage(
                source="system",
                content=f"无法访问noVNC浏览器的地址: http://localhost:{self._browser.novnc_port}/vnc.html",
                metadata={
                    "internal": "no",
        ...
        if self.is_paused:
            yield Response(
                chat_message=TextMessage(
                    content="WebSurfer被暂停， 请恢复以继续访问网页。",
                    source=self.name,
                    ...
                    for action in response:
                        ...
                        if tool_call_name == "answer_question":
                            tool_call_explanation = (
                                "阅读整个网页内容... " + tool_call_explanation
                            )
                        ...
                        self._chat_history.append(
                            AssistantMessage(
                                content=f"当前网页: {action_context}, 我们将采取以下动作: {tool_call_msg}",
                                source=self.name,
                            )
                        )
                        ...
                        if require_approval:
                            ...
                            request_message = TextMessage(
                                source=self.name,
                                content=f"当前网页: {action_context}, 我们将采取以下动作: {action_proposal}",
                            )
                        ...
                        try:
                            ...
                            if "WebSurfer被暂停" in str(e):
                                self.logger.info(f"Tool execution paused: {e}")
                                action_result = "WebSurfer被暂停，动作被中断。"
                            else:
                                self.logger.error(f"Runtime error executing tool: {e}")
                                action_result = f"执行动作{tool_call_msg}时出现错误: {e}"
    ...
    async def _get_llm_response(
        ...
        if not self.single_tab_mode and self._context is not None:
            ...
            tabs_information_str = f"已打开{num_tabs}个标签页(tab)，标签页(tab)信息如下:\n{tabs_information_str}"
        ...
        if focused:
            ...
            focused_hint = f"\n 当前的焦点为: {role}，焦点的ID: {focused} {name}。\n\n"
        ...
        if len(other_targets) > 0:
            ...
            other_targets_str = (
                "未显示的额外有效交互目标(你需要滚动页面来显示并与它们交互)包括:\n"
        ...
        if not self.json_model_output:
            text_prompt = WEB_SURFER_TOOL_PROMPT.format(
                ...
                consider_screenshot="截图是一个浏览器，浏览器正在打开的网页"
    ...
    async def _check_url_and_generate_msg(self, url: str) -> Tuple[str, bool]:
        ...
        if url == "chrome-error://chromewebdata/":
            if self._last_rejected_url is not None:
                ...
                return (
                    f"我不能访问网站 {last_rejected_url}, 因为它不在用户允许访问的网站列表中。",
                    False,
                )

        if self._url_status_manager.is_url_blocked(url):
            return (
                f"我不能访问网站 {url}，因为它被拉黑(禁止访问)了。",
                False,
            )
        ...
        if not self._url_status_manager.is_url_allowed(url):
            if not self._url_status_manager.is_url_rejected(url):
                ...
                if self.action_guard is not None:
                    ...
                    request_message = TextMessage(
                        source=self.name,
                        content=f"网站 {url} 被禁止访问。是否允许在当前会话中访问域名 {domain} ",
                    )
            ...
            return (
                f"我不能访问网站 {url}，因为它不在用户允许访问的网站列表中。",
                False,
            )
        return "", True
    ...
    async def _execute_tool_scroll_down(self, args: Dict[str, Any]) -> str:
        ...
        return f"我在浏览器中向下滚动了{pixels}个像素。"
    ...
    async def _execute_tool_scroll_up(self, args: Dict[str, Any]) -> str:
        ...
        return f"我在浏览器中向上滚动了{pixels}个像素。"
    ...
    async def _execute_tool_click(
        ...
        action_description = (
            f"我点击了'{target_name}'。" if target_name else "我点击了'control'。"
        )
        assert self._context is not None
        assert self._page is not None
    ...
    async def _execute_tool_click_full(
        ...
        action_description = (
            f"我通过按钮'{button}'点击了'{target_name}'，并保持了{hold}seconds。"
            if target_name
            else f"我通过按钮'{button}'点击了'control'，并保持了{hold}seconds。"
        )
        assert self._context is not None
        assert self._page is not None
        new_page_tentative = await self._playwright_controller.click_id(
    ...
    async def _execute_tool_hover(
        ...
        action_description = (
            f"我将鼠标悬浮到'{target_name}'上。"
            if target_name
            else "我将鼠标悬浮到'control'上。"
        )
        assert self._page is not None
    ...
    async def _execute_tool_create_tab(self, args: Dict[str, Any]) -> str:
        ...
        if not approved:
            return ret
        assert self._context is not None
        action_description = f"我创建了一个新标签页并导航到'{url}'。"
    ...
    async def _execute_tool_close_tab(self, args: Dict[str, Any]) -> str:
        try:
            ...
            await self._page.bring_to_front()
            self._prior_metadata_hash = None
            return f"我关闭了标签页{tab_index}并切换到了一个相邻的标签页。"
        except (ValueError, TypeError) as e:
            return f"无效的标签页索引: {e}"
    ...
    async def _execute_tool_upload_file(
        ...
        if target_name:
            action_description = (
                f"我上传了文件'{file_path}'到'{target_name}'。"
            )
        else:
            action_description = f"我上传了文件'{file_path}'。"
        assert self._page is not None
    ...
    async def _execute_tool_keypress(self, args: Dict[str, Any]) -> str:
        ...
        """
        assert self._page is not None, "浏览器页面未初始化"
        keys = args["keys"]
        await self._playwright_controller.keypress(self._page, keys)
        keys_str = ", ".join(f"'{k}'" for k in keys)
        return f"我按顺序按下了以下按键: {keys_str}"
    ...
    async def _execute_tool(
        ...
        """
        assert self._context is not None, "浏览器的context未初始化"
        assert len(message) == 1, "一次只能执行一个 function call"
        assert self._page is not None
        ...
        try:
            ...
            if self._last_download is not None and self.downloads_folder is not None:
                action_description += f"\n\n成功下载了'{self._last_download.suggested_filename}'到本地路径: {self.downloads_folder}"
        except asyncio.CancelledError:
            ...
            return f"WebSurfer被暂停，动作'{name}'被取消。"
        finally:
