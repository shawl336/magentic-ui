...
- 工具名字："web_search", tool_args: {{"query": str, "require_approval": bool}} - 通过web搜索引擎进行搜索。"query"参数表示被搜索内容。
...
帮助提示，保证网页操作执行成功：
    ...
    - 非常重点注意：如果某个操作出错或者失败，不要重复这个动作
    - 填表单时，确保滑动网页窗口来保证你填完了完整的表单
    - 有些时候使用web_search工具搜索做某件事情的大致方法比搜索具体的细节更有效
