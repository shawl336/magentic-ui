...
import shutil
from typing import AsyncGenerator, List, Literal, Sequence, Optional
import re
...
async def _coding(
    ...
    coding_tool = None
    for tool in tools:
        if tool.name == coding_provider:
            coding_tool = tool
            break
    else:
        raise ValueError(f"Coding tool {coding_provider} not found")

    coding_tool_parameters = coding_tool.parameters
    if "request" not in coding_tool_parameters:
        raise ValueError(f"Coding tool {coding_provider} parameters are not valid, the 'request' argument is required")
    assert coding_tool is not None
    create_result = await workbench.call_tool(
        name=coding_tool.name,
        arguments={
            "request": token_limited_context,
        },
    )   
...
class CodingAgent(BaseChatAgent, Component[CodingAgentConfig]):
    ...
    * 例子只是为了给抽象的语言描述做一个大致的具体说明，实际上你需要根据上下文和自身的知识做出最合适的回答。
    ...
    - 当被要求解决数学或逻辑问题时，首先尝试不写代码直接回答，只有在有必要的时候才使用代码（优先使用Python）来解决这类问题。
