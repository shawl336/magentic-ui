...
class Orchestrator(BaseGroupChatManager):
    ...
    def _get_progress_ledger_prompt(
        ...
        additional_instructions = ""
        if self._config.autonomous_execution:
            additional_instructions = "非常重要: 下一个智能体的名字不能是用户或user_proxy，使用任何其他智能体。"
                ...
                try:
                    ...
                    if validate_json(json_response):
                        ...
                        await self._log_message(
                            f"JSON响应的验证失败: {json_response}, 正在重试 ({retries}/{self._config.max_json_retries})"
                        )
                    ...
                    await self._log_message(
                        f"JSON响应的解析失败，正在重试 ({retries}/{self._config.max_json_retries})"
                    )
                retries += 1
    ...
    async def handle_start(self, message: GroupChatStart, ctx: MessageContext) -> None:  # type: ignore
        ...
        if (
            ...
            early_stop_message = StopMessage(
                content="这个群组对话已经结束了。", source=self._name
            )
    ...
    async def do_bing_search(self, query: str) -> str | None:
        try:
            ...
            if bing_search_results.combined_content != "":
                bing_results_progress = f"正在阅读{len(bing_search_results.page_contents)}个网页..."
                await self._log_message_agentchat(
        ...
        try:
            if self._web_agent_topic in self._participant_names:
                ...
                if (
                    ...
                    tabs_information_str = f"已打开{num_tabs}个标签页。标签页信息如下:\n{tabs_information_str}"
    ...
    async def _orchestrate_step_planning(
        ...
        if self._state.task == "" and self._state.plan_str == "":
            self._state.task = "任务: " + last_user_message.content
    ...
    async def _orchestrate_step_execution(
        ...
        if not is_sentinel_step:
            ...
            if not valid_next_speaker:
                raise ValueError(
                    f"从计划中获取的下一个智能体无效: {next_speaker}，团队成员有: {self._agent_execution_names}"
                )
        else:
            assert isinstance(current_step, SentinelPlanStep)
            await self._log_message_agentchat(
                f"执行哨兵步骤: {current_step.title}",
    ...
    async def _replan(self, reason: str, cancellation_token: CancellationToken) -> None:
        ...
        plan_response["plan_summary"] = "重新计划: " + plan_response["plan_summary"]
    ...
    async def _prepare_final_answer(
        ...
        message = TextMessage(
            content=f"最终回复: {final_answer}", source=self._name
        )
    ...
    async def _execute_sentinel_step(
        ...
        if not valid_agent:
            raise ValueError(
                f"无效的智能体: {agent_name}, 团队成员有: {self._agent_execution_names}"
            )
        ...
        while True:
            try:
                ...
                suggested_sleep_duration_reason = "没有提供原因"
                ...
                # For string condition, check with an LLM if the condition is met
                    ...
                    step_description: str = f"步骤标题 {step.title}, 步骤指令 {step.details}, 步骤条件 {step.condition}, 智能体名字 {agent_name}"
                    ...
                    suggested_sleep_duration_reason = response_json.get(
                        "sleep_duration_reason", "没有提供原因"
                    )

                # If condition met, return to complete the step
                if condition_met:
                    log_msg = f"条件已满足: {reason}。"
                    await self._log_message_agentchat(
                        log_msg,
                    ...
                    self._state.message_history.append(
                        TextMessage(
                            content=f"哨兵步骤 '{step.title}' 成功完成。原因: {reason}",
                            source="user",
                        )
                    )
                    return
                    ...
                    await self._log_message_agentchat(
                        f"(检查 #{iteration}) 条件不满足: {reason} \n 继续睡眠{sleep_duration}s直到下一次检查。",
                        metadata={"internal": "no", "type": "sentinel_sleep"},
                    )
                    if self._config.sentinel_plan.dynamic_sentinel_sleep:
                        await self._log_message_agentchat(
                            f"选择该睡眠时间的原因: {suggested_sleep_duration_reason}",
                            metadata={"internal": "no", "type": "sentinel_sleep"},
                        )
            ...
            # exception
                ...
                trace_logger.error(f"哨兵步骤执行出现错误: {e}")
                raise
